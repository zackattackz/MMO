...
test("two connections are established, first connection receives playerJoined event and adds the new player", () => {
    ...
    socket.on("playerJoined", players => {
        socket1players = players;
        let socket1id = socket.id;
        let socket2id = socket2.id;
        expect(socket1players).toBe({
            socket1id: {name:"guest", y: 300, isActive: false},
            socket2id: {name:"guest", y: 300, isActive: false}
        })
    });
...
test("updateY will change player's y and then send a playerMoved event", () => {
    ...
    socket2.on("playerMoved", players => {
        expect(players[socket.id]).toBe({name: "guest", isActive: false, y:50})//expect socket1 y to have changed
    });
});

test("updateIsActive will change player's y and then send a playerMoved event", () => {
    let socket = require('socket.io-client')('http://localhost:8081');
    let socket2 = require('socket.io-client')('http://localhost:8081'); //initialize a second player

    socket.emit("updateIsActive", true);
    socket2.on("playerMoved", players => {
        expect(players[socket.id]).toBe({name: "guest", isActive: true, y:300})//expect socket1 y to have changed
    });
});
